...
- **HTML 导出** — 对话现在可以导出为单个自包含的 HTML 文件，采用响应式布局并支持亮色、暗色和跟随系统三种主题；代码块保留语法高亮和复制按钮，数学公式使用原生 MathML 渲染，打印时始终使用浅色。
- **默认导出格式** — 设置新增“默认导出格式”选项，导出快捷键（Ctrl+Shift+E）和一键导出将使用偏好的格式（Markdown / JSON / TXT / HTML），不再固定为 Markdown。

### ✨ 功能优化

- **导出菜单统一** — 工具箱的导出按钮现在与对话列表共用同一套格式菜单（Markdown / JSON / TXT / HTML / 复制为 Markdown / 分段导出），复制为 Markdown 统一归入导出菜单。
- **HTML 导出：思维链可折叠** — HTML 导出中的思考/推理内容默认折叠为“思考过程”开关，读者先看回答，需要时再展开推理内容。
- **HTML 导出：问答区分更清晰** — 用户消息获得更深的底色、左侧强调条和角色色标签，一眼分清提问与回答。
