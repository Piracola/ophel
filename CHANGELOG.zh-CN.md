...
- **HTML 导出** — 对话现在可以导出为单个自包含的 HTML 文件，采用响应式布局并支持亮色、暗色和跟随系统三种主题；代码块保留语法高亮和复制按钮，数学公式使用原生 MathML 渲染，思维链默认折叠为「思考过程」，打印时始终使用浅色。
- **默认导出格式** — 设置新增「默认导出格式」选项，导出快捷键和一键导出将使用偏好的格式（Markdown / JSON / TXT / HTML），不再固定为 Markdown。
